...
Given(/^Draft datasets by someone else exist titled:$/) do |table|
  table.headers.each do |header|
    dataset = FactoryGirl.create(:dataset, title: header, depositor_name: "Victor Cedarstaff", depositor_email: "other@mailinator.com")
...
Given(/^Datasets published by someone else exist titled:$/) do |table|
  table.headers.each do |header|
    dataset = FactoryGirl.create(:dataset, title: header, publication_state: "released", depositor_name: "Victor Cedarstaff", depositor_email: "other@mailinator.com")
    dataset.identifier =  "10.5027/FK2#{dataset.key}_V1"
    dataset.save
    creator = FactoryGirl.create(:creator, dataset_id: dataset.id)
  end
