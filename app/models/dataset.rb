...
class Dataset < ActiveRecord::Base
  ...
  def recordtext
    ...
    content = "##########\n"
    content = content + "# Dataset Record for doi:#{self.identifier} generated by Illinois Data Bank at #{Time.now.strftime('%Y-%m-%d_%H-%M')}.\n"
    content = content + "##########\n\n"
    ...
    if self.description && self.description != ''
      content = content + "Description => #{self.plain_text_citation}\n\n"
    end
    ...
    content = content +  "\n#{"File".pluralize(self.datafiles.count)} => \n"

    self.datafiles.each do |datafile|
      content = content + ". #{datafile.bytestream_name}"
    end
