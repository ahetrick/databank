...
module Versionable
  ...
  def related_version_entry_hash
    ...
    {version: self_version, selected: false, doi: identifier || "not yet set", version_comment: version_comment || "", publication_date: self.release_date ? self.release_date.iso8601 : "not yet set"}
  end
  ...
  def previous_idb_dataset
    ...
    if previous_version_related_material&.uri
      previous_idb_dataset = Dataset.find_by(identifier: previous_version_related_material.uri)
  ...
  def next_idb_dataset
    next_version_related_material = related_materials.find_by(datacite_list: Databank::Relationship::PREVIOUS_VERSION_OF)

    if next_version_related_material&.uri
      next_idb_dataset = Dataset.find_by(identifier: next_version_related_material.uri)
