class DatabankMailer < ActionMailer::Base
  ...
  def confirm_deposit_update(dataset_key)
    @dataset = Dataset.where(key: dataset_key).first
    if @dataset
      mail(to: [@dataset.depositor_email, @dataset.corresponding_creator_email], cc: 'databank@library.illinois.edu', subject: '[Illinois Data Bank] Dataset successfully updated')
    else
